...
func TestShowTransaction(t *testing.T) {
	net, objs := networkWithTransactionObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowTransaction(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetTransactionResponse
...
func TestListTransaction(t *testing.T) {
	net, objs := networkWithTransactionObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListTransaction(), args)
			require.NoError(t, err)
			var resp types.QueryAllTransactionResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
