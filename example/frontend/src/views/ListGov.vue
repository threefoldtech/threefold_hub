  ...
  <v-container>
    ...
    <v-data-table :headers="headers" :items="proposals">
      ...
      <template v-slot:[`item.final_tally_result`]="{ item }">
        <br />
        <p>
          YES: {{ item.final_tally_result.yes }} <br />
          NO: {{ item.final_tally_result.no }} <br />
          NO_WITH_VETO: {{ item.final_tally_result.no_with_veto }} <br />
          ABSTAIN: {{ item.final_tally_result.abstain }}
        </p>
      </template>
      ...
      <template v-slot:[`item.details`]="{ item }">
        <v-btn
          color="primary"
          @click="$router.push('/gov/' + item.proposal_id)"
        >
          View Details
        </v-btn>
      </template>
